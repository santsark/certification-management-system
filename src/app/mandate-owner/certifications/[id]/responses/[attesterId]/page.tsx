...
export default function ResponseViewPage({ params }: { params: Promise<{ id: string; attesterId: string }> }) {
    const resolvedParams = use(params);
    const certId = resolvedParams.id;
    const attesterId = resolvedParams.attesterId;
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await fetch(`/api/mandate-owner/certifications/${certId}/responses/${attesterId}`);
    ...
    if (!data) return <div>Response not found</div>;

    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="flex items-center justify-between print:hidden">
                <div className="flex items-center gap-4">
                    <Link href={`/mandate-owner/certifications/${certId}/responses`}>
            ...
            <Card>
                <CardHeader className="pb-4">
                    <div className="flex justify-between items-start">
                        <div>
                            <CardTitle>{data.attester.name}</CardTitle>
                            <CardDescription>{data.attester.email}</CardDescription>
                        </div>
